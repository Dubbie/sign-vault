...
class EngagementTrackingTest extends TestCase
{
    ...
    public function test_preview_view_endpoint_records_a_separate_view_type(): void
    {
        [$folder] = $this->createPublicFolderWithSign();

        $this->postJson("/api/public/folders/{$folder->public_slug}/preview-view")->assertNoContent();
        $this->postJson("/api/public/folders/{$folder->public_slug}/preview-view")->assertNoContent();
    ...
    public function test_private_folders_cannot_be_tracked(): void
    {
        ...
        $this->postJson("/api/public/folders/{$folder->public_slug}/preview-view")->assertNotFound();
    ...
    public function test_admin_engagement_endpoint_reports_aggregated_stats(): void
    {
        ...
        $response = $this->getJson('/api/admin/engagement?days=30')
            ...
            ->assertJsonPath('summary.folder_previews', 1)
        ...
        $response->assertJsonStructure([
            'timeseries' => ['folder_full_views', 'folder_previews', 'sign_copies'],
        ]);
    }
}
